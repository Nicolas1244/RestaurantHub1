...
import { X, Trash2, Clock, Calendar, Plus, AlertTriangle, Heart, Calendar as CalendarIcon, Scissors } from 'lucide-react';
...
const DailyEntryModal: React.FC<DailyEntryModalProps> = ({
  ...
  const [isHolidayWorked, setIsHolidayWorked] = useState<boolean>(false);
  const [holidayStart, setHolidayStart] = useState<string>('09:00');
  const [holidayEnd, setHolidayEnd] = useState<string>('17:00');
  ...
  useEffect(() => {
    if (isOpen && employee) {
      ...
      if (absenceShift && absenceShift.status) {
        ...
        // Check if it's a worked holiday
        if (absenceShift.status === 'PUBLIC_HOLIDAY' && absenceShift.isHolidayWorked) {
          setIsHolidayWorked(true);
          setHolidayStart(absenceShift.start || '09:00');
          setHolidayEnd(absenceShift.end || '17:00');
        } else {
  ...
  }, [isOpen, employee, day, existingShifts]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (activeTab === 'shifts') {
      ...
      // Special handling for worked holiday (PUBLIC_HOLIDAY)
      if (selectedAbsence === 'PUBLIC_HOLIDAY' && isHolidayWorked) {
        // Create a worked holiday shift
        const holidayShift = {
          restaurantId,
          employeeId: employee.id,
          day,
          ...
          color: '#DC2626' // Red color for holiday
        };
        
        onSaveAbsence(holidayShift);
      } else {
        // Create regular absence shift
        const absenceShift = {
          restaurantId,
          employeeId: employee.id,
          day,
        ...
        >
          ...
          <form onSubmit={handleSubmit}>
              ...
              <div className="space-y-4">
                ...
                {activeTab === 'absence' && (
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {Object.entries(DAILY_STATUS).map(([status, { label, color }]) => (
                        <button
                          ...
                          type="button"
                          onClick={() => {
                            setSelectedAbsence(status as DailyStatus);
                            // Reset holiday worked flag when changing absence type
                    ...
                    {/* Special handling for Férié 1er Mai */}
                      ...
                      <div className="mt-4 p-4 bg-red-50 rounded-lg border border-red-200">
                        <div className="flex items-center justify-between mb-3">
                          <div className="flex items-center">
                            ...
                            <span className="text-sm font-medium text-red-700">
                              {i18n.language === 'fr' ? 'Férié travaillé (majoré)' : 'Worked holiday (with premium)'}
                            </span>
                          </div>
                        ...
                        {isHolidayWorked && (
                          <div className="mt-3">
                            ...
                            {/* Time inputs for worked holiday */}
                            <div className="grid grid-cols-2 gap-3">
                              <div>
                            ...
                            <div className="mt-2 text-sm text-red-600 flex items-center">
                              ...
                              {calculateTimeInHours(holidayStart, holidayEnd).toFixed(1)}h {i18n.language === 'fr' ? 'majorées à 100%' : 'with 100% premium'}
                            </div>
                          </div>
                        )}
                      </div>
            ...
            <div className="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
              ...
              {existingShifts.some(s => s.employeeId === employee.id && s.day === day) && (
                <button
                  type="button"
